...
var botClient = new TelegramBotClient(Environment.GetEnvironmentVariable("API_KEY"));
...
async Task HandleUpdateAsync(ITelegramBotClient bot, Update update, CancellationToken token)
{
    ...
    if(normalized == "/start")
    {
        await bot.SendMessage(userID, "Добро пожаловать в мир Guns4Cards!\n\nЭтот бот позволит вам собирать коллекцию из десятков разных карточек с оружием, зарабатывать очки," +
            ...
            "Также если вы владелец чата, можете добавить его к себе, ведь чем больше игроков, тем веселее, а также это позволит боту развиваться быстрее!\n\n" +
            "Первым шагом предлагаем ввести /guncommands, чтобы посмотреть список доступных команд бота");
    }
